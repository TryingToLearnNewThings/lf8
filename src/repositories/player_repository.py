...
class PlayerRepository(DatabaseHelper):
    ...
    def Update_player_field(self, update_field, new_value):
        self.Update_field_value(
            "Player", update_field, new_value, self.player_id, "playerID"
        )
    ...
    def Update_high_score(self, player_id, new_score):
        current_score = self.Get_value_from_table(
            "Player", "playerScore", "playerID", self.player_id
        )
        if current_score == None or new_score > current_score:
            self.Update_field_value(
                "Player", "playerScore", new_score, self.player_id, "playerID"
            )
    ...
    def Achievment_player_info(self):
        self.cursor.execute(
            ...
            (
                self.player_id,
            ),  # Add the comma here so that it is interpreted as a tuple.
        )
